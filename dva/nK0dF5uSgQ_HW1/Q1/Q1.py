...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        connection.request("GET", f"/3/movie/{movie_id}/credits?language=en-US&api_key={self.api_key}")
        response = connection.getresponse()
        # print("Status: {} and reason: {}".format(response.status, response.reason))
        if response.status >= 200 and response.status < 300:
            cast_dict = json.loads(response.read().decode())
            if cast_dict["cast"] == None:
                return "NO_CAST"
            cast_dict = sorted(cast_dict["cast"], key=lambda d: d['order'])
            return_list = []
            if exclude_ids != None:
                ...
                exclude_ids_set = set()
            if limit != None:
                for i in range(min(len(cast_dict),limit)):
                    if cast_dict[i]["id"] in exclude_ids_set:
                        continue
                    return_list.append(cast_dict[i])
            else:
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        """
        connection = http.client.HTTPSConnection("api.themoviedb.org")
        connection.request("GET", f"/3/person/{person_id}/movie_credits?language=en-US&api_key={self.api_key}")
        response = connection.getresponse()
        # print("Status: {} and reason: {}".format(response.status, response.reason))
        if response.status >= 200 and response.status < 300:
            movie_creds = json.loads(response.read().decode())["cast"]
            movie_creds = [creds for creds in movie_creds if (vote_avg_threshold == None or (vote_avg_threshold != None and vote_avg_threshold <= creds["vote_average"]))]
            return movie_creds
        raise Exception
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='3618747aee077f2125c300a15a784420')
    seen_name_ids = {2975}
    seen_name_edges = set()

    #base
    movies_creds = tmdb_api_utils.get_movie_credits_for_person('2975', 8)
    for movie_c in movies_creds:
        movie_cast_dets = tmdb_api_utils.get_movie_cast(str(movie_c["id"]), 3, list(seen_name_ids))
        if movie_cast_dets == "NO_CAST":
            continue
        for cast_dets in movie_cast_dets:
            if cast_dets["id"] not in seen_name_ids:
                graph.add_node(str(cast_dets["id"]), cast_dets["name"].replace(",",""))
                seen_name_ids.add(cast_dets["id"])
            if not ((2975, cast_dets["id"]) in seen_name_edges or (cast_dets["id"], 2975) in seen_name_edges):
                graph.add_edge('2975', str(cast_dets["id"]))
                seen_name_edges.add((2975, cast_dets["id"]))
                seen_name_edges.add((cast_dets["id"], 2975))

    print("base")

    # first loop more loops
    newly_seen = set()
    copy_of_seen = seen_name_ids.copy()
    for node_id in copy_of_seen:
        if node_id == 2975:
            continue
        movies_creds = tmdb_api_utils.get_movie_credits_for_person(str(node_id), 8)
        for movie_c in movies_creds:
            movie_cast_dets = tmdb_api_utils.get_movie_cast(str(movie_c["id"]), 3, [2975, node_id]) #this is problem
            if movie_cast_dets == "NO_CAST":
                continue
            for cast_dets in movie_cast_dets:
                if cast_dets["id"] not in seen_name_ids:
                    graph.add_node(str(cast_dets["id"]), cast_dets["name"].replace(",",""))
                    seen_name_ids.add(cast_dets["id"])
                    newly_seen.add(cast_dets["id"])
                if not (((node_id, cast_dets["id"]) in seen_name_edges) or ((cast_dets["id"], node_id) in seen_name_edges)):
                    graph.add_edge(str(node_id), str(cast_dets["id"]))
                    seen_name_edges.add((node_id, cast_dets["id"]))
                    seen_name_edges.add((cast_dets["id"], node_id))
        print("node added 1")

    #second loop more loops
    for node_id in newly_seen:
        movies_creds = tmdb_api_utils.get_movie_credits_for_person(str(node_id), 8)
        for movie_c in movies_creds:
            movie_cast_dets = tmdb_api_utils.get_movie_cast(str(movie_c["id"]), 3, [node_id])
            if movie_cast_dets == "NO_CAST":
                continue
            for cast_dets in movie_cast_dets:
                if cast_dets["id"] not in seen_name_ids:
                ...
                if not (((node_id, cast_dets["id"]) in seen_name_edges) or ((cast_dets["id"], node_id) in seen_name_edges)):
                    ...
                    seen_name_edges.add((node_id, cast_dets["id"]))
                    seen_name_edges.add((cast_dets["id"], node_id))
        print("node added 2", node_id)

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK

    # print(tmdb_api_utils.get_movie_cast(28, 2, [8349]))
